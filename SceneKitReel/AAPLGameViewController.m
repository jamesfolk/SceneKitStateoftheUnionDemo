...
- (void)setupSceneElements
{
    ...
    SCNNode *baseboardNode = [SCNNode nodeWithGeometry:[SCNPlane planeWithWidth:800 height:8]];
...
#define TITLE_SIZE (TEXT_SCALE*0.45)
    ...
    sceneKitLogo.position = SCNVector3Make(200, LOGO_SIZE/2, 1050);
    ...
    //slide 2
    SCNText *text = [SCNText textWithString:@"Load\n3D Scenes" extrusionDepth:5];
    text.chamferRadius=0.3;
    text.flatness = 0.1;
#if TARGET_OS_IPHONE
    text.font = [UIFont fontWithName:@"Avenir-heavy" size:30];
#else
    text.font = [NSFont fontWithName:@"Avenir Heavy" size:30];
#endif
    ...
    position = SCNVector3Make(110, -min.y*TEXT_SCALE, position.z);
    textNode.position = position;
    [_introNodeGroup addChildNode:textNode];
    ...
    _shipHandle.position =  SCNVector3Make(200, 0, position.z + 30);
    ...
    //slide 3
    text = [SCNText textWithString:@"Animate & Render" extrusionDepth:5];
    text.chamferRadius=0.3;
    text.flatness = 0.1;
#if TARGET_OS_IPHONE
    text.font = [UIFont fontWithName:@"Avenir-heavy" size:30];
    ...
    position = SCNVector3Make(100, -min.y*TEXT_SCALE, position.z);
    textNode.position = position;
    [_introNodeGroup addChildNode:textNode];
    
    
    //slide 4
    text = [SCNText textWithString:@"OS X    iOS" extrusionDepth:5];
    text.chamferRadius=0.3;
    text.flatness = 0.1;
#if TARGET_OS_IPHONE
    text.font = [UIFont fontWithName:@"Avenir-heavy" size:50];
#else
    text.font = [NSFont fontWithName:@"Avenir Heavy" size:50];
#endif
    text.materials = @[[self textMaterial]];
    textNode = [SCNNode node];
    textNode.geometry = text;
    ...
    position = SCNVector3Make(200 - (max.x-min.x)*TEXT_SCALE*0.5, -min.y*TEXT_SCALE, position.z);
    textNode.position = position;
    [_introNodeGroup addChildNode:textNode];
    ...
    [SCNTransaction setCompletionBlock:^{
        ...
        sceneKitLogo.geometry.firstMaterial.emission.intensity = 0.75;
...
- (void) nextIntroductionStep
{
    ...
    if (_introductionStep == 2) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [SCNTransaction begin];
            [SCNTransaction setAnimationDuration:1.0];
            _shipNode.opacity = 1.0;
            [_shipNode runAction:[SCNAction repeatActionForever:[SCNAction rotateByX:0 y:M_PI z:0 duration:1.0]]];
            [SCNTransaction commit];
        });
    }
    ...
    if (_introductionStep == 3) {
        ...
        oldSpotTransform = _spotLightNode.transform;
        ...
        SCNAction *action = [SCNAction sequence:@[[SCNAction waitForDuration:1],[SCNAction repeatActionForever:[SCNAction rotateByX:0 y:M_PI z:0 duration:2]]]];
        ...
        [_shipPivot.parentNode addAnimation:animation forKey:nil];
        ...
        _shipHandle.position = SCNVector3Make(_shipHandle.position.x, _shipHandle.position.y, _shipHandle.position.z-TEXT_Z_SPACING-50);
    }
    ...
    if (_introductionStep == 5) {
        ...
        _ambientLightNode.light.color = [SKColor colorWithWhite:0.3 alpha:1.0];
        _spotLightNode.position = SCNVector3Make(0, 0, 0);
        _spotLightNode.eulerAngles = SCNVector3Make(-M_PI_4,0,0);
        _cameraNode.position = SCNVector3Make(0, 0, 120);
...
- (void)showParticlesSlide
{
    ...
    [_cameraHandle runAction:[SCNAction sequence:@[[SCNAction waitForDuration:2],tr,[SCNAction runBlock:^{
        ps.birthRate = 300;
    }]]]];
}
...
#define W 50
    ...
    _torus.physicsBody = [SCNPhysicsBody staticBody];
    _torus.opacity = 0.0;
...
- (void)showShadersSlide
{
    ...
    [node runAction:[SCNAction repeatActionForever:[SCNAction rotateByX:0 y:M_PI z:0 duration:4.0]]];
    
    NSString *geoModifier = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"sm_geom" ofType:@"shader"] encoding:NSUTF8StringEncoding error:nil];
    ...
    NSString *lighting = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"sm_light" ofType:@"shader"] encoding:NSUTF8StringEncoding error:nil];

    node.geometry.shaderModifiers = @{SCNShaderModifierEntryPointGeometry : geoModifier,
                                      SCNShaderModifierEntryPointSurface : surfaceModifier,
                                      SCNShaderModifierEntryPointFragment: modifier,
                                      SCNShaderModifierEntryPointLightingModel: lighting,
                                      };
...
#if 0
            if (_step > 0 && _step != 3 && (shouldGoBack==NO || _step > 1)) {
                //particles
                SKEmitterNode * burst = [NSKeyedUnarchiver unarchiveObjectWithFile:[[NSBundle mainBundle] pathForResource:@"starBurst" ofType:@"skn"]];
                burst.particleBirthRate = 0;
                burst.particleScale = 1.5;
                [node addChild:burst];
                [burst runAction:[SKAction sequence:@[
                                                      [SKAction runBlock:^{burst.particleBirthRate = 10;}],
                                                      [SKAction waitForDuration:0.15],
                                                      [SKAction runBlock:^{burst.particleBirthRate = 0;}],
                                                      [SKAction waitForDuration:burst.particleLifetime],
